...
fn is_palindrome(word: &str) -> bool {
    let stripped = word.chars().filter(|c| c.is_alphanumeric()).collect::<String>();
    let reversed = reverse_string(&stripped.to_lowercase());
    stripped.to_lowercase() == reversed
}
...
mod tests {
    ...
    fn test_count_char_occurrences() {
        ...
        assert_eq!(count_char_occurrences("Hello", 'x'), 0); 
        assert_eq!(count_char_occurrences("", 'a'), 0); 
    }
}
